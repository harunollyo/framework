...
use Framework\Scheduler\Repositories\QueueRepository;
use Exception;
...
class Runner
{
     ...
     *
     * @param QueueRepository $repository The repository.
     *
     * @return void
     *
     * @since 1.0.0
    ...
    {
        if (empty($job)) {
            throw new Exception("Invalid job provided to resolve");
        }

        if (empty($job->resolver)) {
            throw new Exception("Missing resolver class");
        }
    }
    ...
    protected function make_resolver(string $resolver)
    {
        if (!class_exists($resolver)) {
            throw new Exception(sprintf('Class [%s] missing to resolve the job', $resolver));
        }

        if (!method_exists($resolver, 'handle')) {
            throw new Exception(sprintf('Missing [%s::handle] method to resolve the job', $resolver));
        }

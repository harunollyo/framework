...
The session is saved at each point the framework sends a response — site route dispatch, redirects, and REST dispatch — plus a `shutdown` hook that catches admin, AJAX, and template paths. These overlap by design: a redirect passes through three of them. Saving is idempotent — a session that never started, one that has not changed, or one whose flash data has already been aged into storage writes nothing.

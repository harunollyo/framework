...
if wp plugin is-active framework --path="${WP_PATH}" --allow-root 2>/dev/null; then
    echo "Plugin framework is already active."
else
    echo "Activating framework plugin..."
    wp plugin activate framework --path="${WP_PATH}" --allow-root
fi
